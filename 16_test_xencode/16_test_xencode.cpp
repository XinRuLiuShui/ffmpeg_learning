...
int main(int argc, char* argv[])
{
	...
	int count = 0;
	XEncode ec;
	auto c = ec.Creat(codec_id);
	c->width = 400;
	c->height = 300;
	ec.set_c(c);
	ec.SetOpt("crf", 18);//恒定速率因子(crf)
	if (ec.Open())
	{
		cout << "encoder Open Success!" << endl;
	}
	else
	{
	...
	auto frame = ec.CreatFrame();
	if (!frame)
	{
		cout << "Creat frame Failed!" << endl;
	}
	...
	//		///分析NALU
	//		///一个AVPacket中包含多个NALU以0001间隔，多个是以001间隔
	//		//0001[NAL_HEAD]
	//		//	[NAL HEAD]
	//		//1个字节orbiddenbit（1bit），nalreference_bit（2bits）（优先级）
	//		///nal_unit_type(5bits)
